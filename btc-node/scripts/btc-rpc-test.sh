...
RPC_URL="${BTC_RPC_URL:-http://100.125.134.45:8332/}"
...
选项:
  --url URL       RPC 地址（默认: http://100.125.134.45:8332/）
  --user USER     rpcuser（或环境变量 BTC_RPC_USER）
  --pass PASS     rpcpassword（或环境变量 BTC_RPC_PASS）
  -h, --help      显示帮助

凭证: 命令行 > 环境变量 > btc-node/config/bitcoin.conf

示例:
  bash scripts/btc-rpc-test.sh --url http://127.0.0.1:8332/
  bash scripts/btc-rpc-test.sh --url http://100.125.134.45:8332/ --user ... --pass ...

注意: 远程访问需在 bitcoin.conf 配置 rpcallowip（如 Tailscale 100.64.0.0/10）。
EOF
}
...
main() {
  ...
  rpc_check "getblockchaininfo" getblockchaininfo '[]' chain_main
